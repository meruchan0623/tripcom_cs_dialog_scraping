...
def export_structured_via_cdp_proxy(
    ...
    detail_script = (repo_root / "detail-page.js").read_text(encoding="utf-8")
    ...
    for i, sess in enumerate(sessions, start=1):
        ...
        target_id = ""
        try:
            log(f"[{i}/{len(sessions)}] 结构化(CDP): {sess.session_id}")
            target_id = proxy.new_tab(sess.detail_url)
            proxy.eval(target_id, f"(() => {{ {detail_script}\nreturn true; }})()", timeout=30)
            meta = {"sessionId": sess.session_id, "csName": sess.cs_name, "createTime": sess.create_time}
            script = f"""
(async () => {{
  const dp = window.__IM_ARCHIVE_DETAIL_PAGE__;
  if (!dp) throw new Error('detail-page helper not installed');
  await dp.loadAllMessages({{ settleMs: 400, stableRounds: 3 }});
  return await dp.extractConversationStructured({json.dumps(meta, ensure_ascii=False)});
}})()
"""
            data = proxy.eval(target_id, script, timeout=180)
            ...
            append_failure(
                failures_file,
                {"kind": "structured", "session_id": sess.session_id, "cs_name": sess.cs_name, "error": str(exc)},
            )
